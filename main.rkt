...
;; If the student executable `exe-path` exists, run it by calling `run-thunk` in the executable's directory.
;; Either way, record the output of the run or failure diagnostics to the `out-file`.
(define (test-student-executable exe-path run-thunk out-file)
  (define (write-team-output str)
    (with-output-to-file out-file (lambda () (displayln str))))

  (unless (file-exists? out-file)
    (log-cs4500-f18-warning "about to test student executable '~a', current ps -f:~n~a" (path->string exe-path) (current-process-list))
    (cond
      [(not (file-exists? exe-path))
       (write-team-output (format "file '~a' does not exist" (path->string exe-path)))]
      [(not (file-executable? exe-path))
       (write-team-output (format "file '~a' is not executable" (path->string exe-path)))]
      [else
       (log-cs4500-f18-warning "executing ~a" (path->string exe-path))
       (define r-str
         (parameterize ((current-directory (path-only exe-path)))
            (run-thunk)))
  ...
  (for ((team (hash-keys commits-data)))
    ...
    (unless (directory-exists? this-r-test)
      ...
      (unless (equal? expected-files actual-in-dir)
        (log-cs4500-f18-info "Extra/Missing test files for ~a" team)
...
(define (fest-matrix results-dir cfg)
  ...
  (define commits-data (file->value (hash-ref cfg team-commits)))
  (for ((team (hash-keys commits-data))
        #:when (and (has-valid-testfest-exe? team results-dir cfg)
                    (not (skip-fest? team results-dir cfg))))
    (log-cs4500-f18-info "testfest '~a' vs ..." team)
    (log-cs4500-f18-warning "Current ps -f:~n~a" (current-process-list))
    (define this-exe (build-path s-root team s-path))
    (define student-r-dir (build-path results-dir team))
    (for ((that-name-str (hash-keys commits-data))
          #:when (and (not (equal? team that-name-str))
      ...
      (unless (file-exists? that.txt)
        (log-cs4500-f18-info "testfest '~a' vs '~a'" team that-name-str)
